...
FUNCTION remove_struct_tags, s, remove
  ...
  ; Case-insensitive remove list
  tags_u   = STRUPCASE(tags)
  remove_u = STRUPCASE(remove)

  keep = WHERE(~IS_MEMBER(tags_u, remove_u), n)
  IF n EQ 0 THEN RETURN, s

  ; Build output starting from the first kept tag
  k0  = keep[0]
  out = CREATE_STRUCT(tags[k0], s.(k0))

  ; Append remaining tags
  FOR i = 1, n-1 DO BEGIN
    k   = keep[i]
    out = CREATE_STRUCT(out, tags[k], s.(k))
  ENDFOR
...
pro gx_box_add_refmap_srh, box, fits_file, id = id
  ...
  index = remove_struct_tags(index, ['DSUN_OBS', 'RSUN_OBS'])
